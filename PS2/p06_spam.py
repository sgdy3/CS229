    ...
    for mess in messages:
        ...
        for key in dic:
            if(dic_all.get(key)):
                ...
                dic_all[key]=dic[key]
    dic_all={word:times for word,times in dic_all.items() if(times>5)}
    return dic_all
...
def transform_text(messages, word_dictionary):
    ...
    """
    # *** START CODE HERE ***
    pres=np.zeros((len(messages),len(word_dictionary)))
    word=list(word_dictionary.keys())
    for i in range(len(messages)):
        dic=get_words(messages[i])
        for k,y in dic.items():
            if(word_dictionary.get(k)):
                ind=word.index(k)
                pres[i,ind]+=1
            else:
                continue
...
def get_top_five_naive_bayes_words(model, dictionary):
    ...
    """
    # *** START CODE HERE ***
    word = np.array(list(dictionary.keys()))
    phi_k_y1, phi_k_y0, phi_y = model
    temp=-(np.log(phi_k_y1)-np.log(phi_k_y0))
    seq=np.argsort(temp)
    return  word[seq]
    # *** END CODE HERE ***
...
def main():
    train_messages, train_labels = util.load_spam_dataset(r'E:\python_program\cs229\data\PS2\ds6_train.tsv')
    val_messages, val_labels = util.load_spam_dataset(r'E:\python_program\cs229\data\PS2\ds6_val.tsv')
    test_messages, test_labels = util.load_spam_dataset(r'E:\python_program\cs229\data\PS2\ds6_test.tsv')
