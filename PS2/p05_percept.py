...
def initial_state():
    ...
    # *** START CODE HERE ***
    state={'beta':np.empty(0),'x':np.empty(0)}
    return  state
    # *** END CODE HERE ***
...
def predict(state, kernel, x_i):
    ...
    for i in range(state['x'].shape[0]):
        temp = temp + state['beta'][i] * kernel(state['x'][i], x_i)
    return sign(temp)
    # *** END CODE HERE ***
...
def update_state(state, kernel, learning_rate, x_i, y_i):
    ...
    # *** START CODE HERE ***
    temp=0
    if(state['beta'].shape[0]==0):
        state['beta']=np.append(state['beta'],learning_rate*(y_i-sign(0)))
        state['x']=np.zeros((1,x_i.shape[0]))
        state['x'][0,:]=x_i
    else:
        for i in range(state['x'].shape[0]):
            temp=temp+state['beta'][i]*kernel(state['x'][i],x_i)
        beta=learning_rate*(y_i-sign(temp))
        state['beta']=np.append(state['beta'],beta)
        state['x']=np.insert(state['x'],state['x'].shape[0],x_i,axis=0)
    # *** END CODE HERE ***
...
def train_perceptron(kernel_name, kernel, learning_rate):
    ...
    """
    train_x, train_y = util.load_csv(r'E:\python_program\cs229\data\PS2\ds5_train.csv')
    ...
    test_x, test_y = util.load_csv(r'E:\python_program\cs229\data\PS2\ds5_test.csv')
